...
public class TimeFormatter {
  ...
  public static SpannableStringBuilder formatTimeRemaining(Context context, double routeDuration) {
    ...
    formatDays(context, days, textSpanItems);
  ...
  private static void formatDays(Context context, long days, List<SpanItem> textSpanItems) {
    if (days != 0) {
      String dayString = String.format(" %s ", context.getString(R.string.day));
      String daysString = String.format(" %s ", context.getString(R.string.days));
      String dayFormat = days > 1 ? daysString : dayString;
  ...
  private static void formatHours(Context context, long hours, List<SpanItem> textSpanItems) {
    if (hours != 0) {
      String hourString = String.format(" %s ", context.getString(R.string.hr));
  ...
  private static void formatMinutes(Context context, long minutes, List<SpanItem> textSpanItems) {
    if (minutes != 0) {
      String minuteString = String.format(" %s ", context.getString(R.string.min));
